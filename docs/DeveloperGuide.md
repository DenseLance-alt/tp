...
1. Deleting a customer who has a delivery
   ...
   1. Prerequisites: Ensure the default sample data is loaded. List all customers using the `list` command. The customer at index 1 (David L) has a delivery scheduled from 2026-10-15 to 2026-10-19.

   1. Test case: `delete 1`<br>
      Expected: David L is deleted from the list. Run `find-delivery dt/2026-10-19` to verify David L no longer appears, confirming that their associated delivery was also removed.

### Today's delivery panel

1. Verifying deliveries displayed on launch

   1. Prerequisites: Launch the app with the default sample data.

   1. Observe the delivery panel on the right.<br>
      Expected: The panel header shows today's date. Customers with a delivery scheduled for today are listed, sorted by delivery time. If no deliveries are scheduled for today, the panel is empty.

1. Panel is unaffected by filter commands
...
1. Finding customers by a combination of filters

   1. Prerequisites: List all customers using the `list` command. Multiple customers in the list.

   1. Test case: `find n/David a/Serangoon`<br>
      Expected: Only customers whose name contains `David` **and** whose address contains `Serangoon` are shown.

   1. Test case: `find n/David Roy`<br>
      Expected: Customers whose name contains `David` **or** `Roy` are shown.

   1. Other incorrect commands to try: `find` (no filters), `find n/` (empty keyword)<br>
...
1. Finding customers by exact delivery date
   ...
   1. Test case: `find-delivery dt/2026-03-23`<br>
...
1. Invalid `find-delivery` commands
   ...
   1. Test case: `find-delivery dt/2026-04-01 st/2026-04-01 ed/2026-04-30`<br>
      Expected: Error message shown. (`dt/` cannot be used together with `st/` and `ed/`.)

   1. Test case: `find-delivery st/2026-10-30 ed/2026-10-15`<br>
      Expected: Error message shown. (Start date must not be after end date.)

   1. Test case: `find-delivery st/2026-10-15`<br>
      Expected: Error message shown. (`st/` must be used together with `ed/`.)

   1. Other incorrect commands to try: `find-delivery dt/2026-13-01` (invalid date), `find-delivery` (no arguments)<br>
      Expected: Error message shown.
...
1. Finding customers whose deliveries have expired
   ...
   1. Test case: `expired bf/2026-03-30`<br>
...
1. Scheduling a delivery for a customer without an existing delivery
   ...
   1. Prerequisites: List all customers using the `list` command. Identify a customer without a delivery (e.g. Irfan Ibrahim at index 5 in the default sample data).

   1. Test case: `schedule 5 st/2026-05-01 ed/2026-05-31 tm/12:00 d/135`<br>
      Expected: Delivery details appear on the customer's card. Success message is shown.

1. Scheduling a delivery for a customer who already has one

   1. Prerequisites: Customer at index 5 has a delivery (from the previous test case).

   1. Test case: `schedule 5 st/2026-06-01 ed/2026-06-30 tm/14:00 d/2`<br>
      Expected: No delivery added. Error message shown.

   1. Other incorrect commands to try: start date after end date, delivery day out of range (e.g. `d/8`), `24:00` as delivery time<br>
      Expected: Error message shown.
...
1. Rescheduling the delivery of a customer who has one
   ...
   1. Prerequisites: Customer at index 5 has a delivery (continuing from the `schedule` test cases above).

   1. Test case: `reschedule 5 ed/2026-06-30`<br>
...
1. Rescheduling the delivery of a customer without one
   ...
   1. Prerequisites: List all customers using the `list` command. Identify a customer without a delivery (e.g. David Li at index 4 in the default sample data).

   1. Test case: `reschedule 4 ed/2026-06-30`<br>
      Expected: No changes made. Error message shown.

   1. Other incorrect commands to try: `reschedule 5` (no fields provided)<br>
      Expected: Error message shown.
...
1. Unscheduling the delivery of a customer who has one
   ...
   1. Prerequisites: Customer at index 5 has a delivery (continuing from the `schedule` or `reschedule` test cases above).

   1. Test case: `unschedule 5`<br>
      Expected: Delivery details are removed from the customer's card. Success message is shown.

1. Unscheduling the delivery of a customer without one

   1. Test case: `unschedule 5` (immediately after the previous test case)<br>
      Expected: No changes made. Error message shown.
